...
/**
 * Handle /apri-ordinazioni Slack slash command
 * Admin-only: Opens ordering for all users
 */
export async function POST(request: Request) {
  ...
  try {
    ...
    if (settings) {
      ...
      const success = await updateSystemSettings(settings.id, true);
      if (!success) {
        return NextResponse.json({
          response_type: "ephemeral",
          text: "Errore durante l'apertura delle ordinazioni. Riprova.",
        });
      }
    } else {
      // Create new settings
    ...
    console.error("Error handling /apri-ordinazioni command:", error);
    return NextResponse.json({
      response_type: "ephemeral",
